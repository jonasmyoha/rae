...
# fixtures (345, 346, 348). Fixtures that expect a parse error do not parse and
# are skipped by the check itself (a parse error under tests/cases is that
# fixture's own business); a parse error under lib/ or examples/ fails. Exits
...
for x in r:
    p = x["path"]
    if x.get("overCap"): bad.append((p, x.get("message", "over cap")))
    elif x["changed"]: bad.append((p, "not canonically formatted"))
