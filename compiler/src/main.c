...
// Tabs in the recipe lines below are real \t — required by make.
// The raylib link line is the same one royalblush-rae uses; it
// works for raylib apps AND for plain-stdlib apps (rae_runtime.c
// references Cocoa for App-Nap opt-out, so even a hello-world
// would otherwise miss -framework Cocoa). Linking raylib too is
// cheap: an unused libraylib.a costs nothing at runtime.
static const char* RAE_INIT_TEMPLATE_MAKEFILE =
  "# Generated by `rae init`. Set RAE_REPO if rae lives somewhere\n"
  "# other than ../rae-lang-dev/rae, e.g. `make RAE_REPO=/abs/path`.\n"
  "\n"
  "SHELL := /bin/zsh\n"
  ".DEFAULT_GOAL := run\n"
  "\n"
  "RAE_REPO ?= $(realpath $(CURDIR)/../rae-lang-dev/rae)\n"
  ...
  "BIN      := $(BUILD)/app\n"
  "\n"
  "RAYLIB_A := /opt/homebrew/lib/libraylib.a\n"
  "CFLAGS   := -O2 -I/opt/homebrew/include -DRAE_HAS_RAYLIB\n"
  "LDFLAGS  := -L/opt/homebrew/lib $(RAYLIB_A) \\\n"
