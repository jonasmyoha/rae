...
extern void rae_ext_drawCylinder(Vector3 position, double radiusTop, double radiusBottom, double height, int64_t slices, Color color);
...
extern int64_t rae_ext_isKeyPressed(int64_t key);
...
RAE_UNUSED static int64_t rae_isOccupied_Game_Int_Int_(Game* g, int64_t x, int64_t y) {
  ...
  return rae_get_List_Int_(&(g->grid), y * g->width + x);
}
...
RAE_UNUSED static void rae_lockPiece_Game_(Game* g) {
  ...
  int64_t gy = pos->y + py;
  if (gy >= 0 && gy < g->height) {
  rae_set_List_Int_T_(grid, gy * g->width + gx, rae_any(1));
  }
  }
  px = px + 1;
  }
...
RAE_UNUSED static void rae_draw_Game_(Game* g) {
  ...
  rae_ext_clearBackground((Color){ .r = 20, .g = 20, .b = 25, .a = 255 });
  int64_t y = 0;
  {
  while (y < g->height) {
  int64_t x = 0;
  {
  while (x < g->width) {
  int64_t val = rae_get_List_Int_(&(g->grid), y * g->width + x);
  if (val > 0) {
  rae_ext_drawRectangle(rae_toFloat_Int_(x * 20 + 100), rae_toFloat_Int_(y * 20 + 50), 18.0, 18.0, (Color){ .r = 100, .g = 100, .b = 255, .a = 255 });
  }
  x = x + 1;
  }
  }
  ...
  while (px < 4) {
  if (rae_getShapeCell_TetrominoKind_Int_Int_Int_(cp->kind, cp->rotation, px, py) == 1) {
  rae_ext_drawRectangle(rae_toFloat_Int_((pos->x + px) * 20 + 100), rae_toFloat_Int_((pos->y + py) * 20 + 50), 18.0, 18.0, (Color){ .r = 255, .g = 100, .b = 100, .a = 255 });
  }
  px = px + 1;
  }
  }
  ...
  rae_ext_drawText(rae_ext_rae_str_concat(rae_ext_rae_str_concat("Score: ", rae_ext_rae_str(g->score)), ""), 10.0, 10.0, 20.0, (Color){ .r = 255, .g = 255, .b = 255, .a = 255 });
  if (g->state == GameState_Paused) {
  rae_ext_drawText("PAUSED", 150.0, 200.0, 40.0, (Color){ .r = 255, .g = 255, .b = 0, .a = 255 });
  }
  if (g->state == GameState_GameOver) {
  rae_ext_drawText("GAME OVER", 100.0, 200.0, 40.0, (Color){ .r = 255, .g = 0, .b = 0, .a = 255 });
  rae_ext_drawText("Press R to Restart", 120.0, 250.0, 20.0, (Color){ .r = 255, .g = 255, .b = 255, .a = 255 });
  }
  rae_ext_endDrawing();
}
...
int main(void) {
  ...
  Game g = (Game){ .state = GameState_Menu, .grid = rae_createList_Int_(200), .width = 0, .height = 0, .currentPiece = { .kind = TetrominoKind_I, .pos = { .x = 0, .y = 0 }, .rotation = 0 }, .score = 0, .moveTimer = 0, .moveDelay = 0 };
  rae_initGame_Game_(&(g));
  {
  while (rae_ext_windowShouldClose() == 0) {
  if (g.state == GameState_GameOver) {
  if (rae_ext_isKeyPressed(82) == 1) {
  rae_clear_List_(&(g.grid));
  rae_initGame_Game_(&(g));
  }
  }
