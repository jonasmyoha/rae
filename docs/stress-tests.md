...
| ~~08~~ | `silentNoOpWrite` | `xs.set(index: 10, value: 5)` on a 3-element list | **FLIPPED to `handles`** (docs/collections.md): a write past the end is `runtime error: List.set: index 10 is out of range for length 3`, exit 70 (reads stay `none`). It used to drop the write silently with exit 0 | Java, Python, Rust, Go throw or panic; JS grows the array (a different foot gun); C writes past the buffer |
