...
- **Creation:** Types that allocate have a constructor: a function named `create` returning the type (or `opt` of it), resolved by the expected type (#880). The older `createType()` names stay as aliases for one release.
