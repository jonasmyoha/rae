...
// RAE_BIOME_ISLAND_RADIUS / RAE_BIOME_ISLAND_FALLOFF are pipeline-creation
// `override` constants (docs/shaders-and-the-compiler.md): the composed shader
// text is static and validated at build, and the island size travels beside
// it as a ShaderConstant (WorldBiome.islandConstants) so it can still grow
// between levels (#41). The defaults match WorldBiome.defaultIsland*.
