...
- [ ] #41 UI Phase 1b: Add `lib/ui/ecs.rae` (`Entity = Int`, `ComponentTable(T)` sparse-set, `UiWorld` with named tables) + `lib/ui/components.rae` (all authored + derived component types from the design doc) + `lib/ui/registry.rae` (component name → deserialiser/applier table). World boots; can `add`/`get` components by hand.
