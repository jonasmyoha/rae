...
- [ ] #42 UI Phase 1c: Add `lib/ui/scene.rae` (`.raescene` JSON → in-memory `Scene` struct, validates `type=="Scene"`/`version==2`/root exists/children exist) and `lib/ui/scene_loader.rae` (Scene → ECS entities + components + ParentLink hierarchy). No layout yet — just dump the entity list.
