...
RAE_UNUSED void rae_ext_drawTextureEx(Texture texture, Vector2 pos, double rotation, double scale, Color tint) {
  DrawTextureEx((Texture){ .id = (unsigned int)texture.id, .width = (int)texture.width, .height = (int)texture.height, .mipmaps = (int)texture.mipmaps, .format = (int)texture.format }, pos, (float)rotation, (float)scale, (Color){ (unsigned char)tint.r, (unsigned char)tint.g, (unsigned char)tint.b, (unsigned char)tint.a });
}
...
RAE_UNUSED void rae_ext_drawRectangleLines(double x, double y, double width, double height, Color color) {
  DrawRectangleLines((int)x, (int)y, (int)width, (int)height, (Color){ (unsigned char)color.r, (unsigned char)color.g, (unsigned char)color.b, (unsigned char)color.a });
}
...
RAE_UNUSED static int64_t rae_elapsedNs_Timer_(Timer* t);
RAE_UNUSED static double rae_elapsedMs_Timer_(Timer* t);
RAE_UNUSED static double rae_elapsedSeconds_Timer_(Timer* t);
...
RAE_UNUSED static Timer rae_createTimer_(void) {
  rae_ext_rae_log_cstr("DEBUG: createTimer");
  return (Timer){ .startTime = 0, .accumulated = 0, .running = 0 };
}

RAE_UNUSED static void rae_start_Timer_(Timer* t) {
  rae_ext_rae_log_cstr("DEBUG: start timer");
  if (t->running) {
  return;
  }
...
RAE_UNUSED static double rae_elapsedMs_Timer_(Timer* t) {
  ...
  return rae_toFloat_Int_(ns) / 1000000.0;
}

RAE_UNUSED static double rae_elapsedSeconds_Timer_(Timer* t) {
  int64_t ns = rae_elapsedNs_Timer_(t);
  return rae_toFloat_Int_(ns) / 1000000000.0;
}

int main(void) {
  rae_ext_initWindow(800, 450, "Rae Easing 2D");
  rae_ext_setTargetFPS(60);
  State s = (State){ .timer = rae_createTimer_(), .animDuration = 2.0 };
  rae_start_Timer_(&(s.timer));
  {
  while (rae_ext_windowShouldClose() == 0) {
  double elapsed = rae_elapsedSeconds_Timer_(&(s.timer));
  if (elapsed > s.animDuration) {
  rae_reset_Timer_(&(s.timer));
  rae_start_Timer_(&(s.timer));
  elapsed = 0.0;
  }
  double t = elapsed / s.animDuration;
  double easedT = rae_easeInOutQuad_Float_(t);
  double startX = 100.0;
  double endX = 700.0;
  double currentX = startX + (endX - startX) * easedT;
  rae_ext_beginDrawing();
  rae_ext_clearBackground((Color){ .r = 245, .g = 245, .b = 245, .a = 255 });
  rae_ext_drawText("Easing: easeInOutQuad", 10.0, 10.0, 20.0, (Color){ .r = 200, .g = 200, .b = 200, .a = 255 });
  rae_ext_drawCircle(currentX, 225.0, 40.0, (Color){ .r = 230, .g = 41, .b = 55, .a = 255 });
  rae_ext_endDrawing();
  }
  }
  rae_ext_closeWindow();
