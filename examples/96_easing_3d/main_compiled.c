...
int main(void) {
  rae_ext_initWindow(800, 450, "Rae Easing 3D");
  rae_ext_setTargetFPS(60);
  Camera3D cam = (Camera3D){ .position = { .x = 10.0, .y = 10.0, .z = 10.0 }, .target = { .x = 0.0, .y = 0.0, .z = 0.0 }, .up = { .x = 0.0, .y = 1.0, .z = 0.0 }, .fovy = 45.0, .projection = 0 };
  Timer timer = rae_createTimer_();
  double animDuration = 3.0;
  rae_start_Timer_(&(timer));
  {
  while (rae_ext_windowShouldClose() == 0) {
  double elapsed = rae_elapsedSeconds_Timer_(&(timer));
  if (elapsed > animDuration) {
  rae_reset_Timer_(&(timer));
  rae_start_Timer_(&(timer));
  elapsed = 0.0;
  }
  double t = elapsed / animDuration;
  double bounceY = rae_easeOutBounce_Float_(t);
  double currentY = 5.0 - 5.0 * bounceY;
  rae_ext_beginDrawing();
  rae_ext_clearBackground((Color){ .r = 245, .g = 245, .b = 245, .a = 255 });
  rae_ext_beginMode3D(cam);
  rae_ext_drawGrid(10, 1.0);
